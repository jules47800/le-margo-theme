                                        ...
                                        <div class="action-buttons">
                                            ...
                                            <?php endif; ?>
                                             <?php if ($is_past && $reservation->status === 'confirmed') : ?>
                                                <a href="<?php echo le_margo_get_action_url('noshow', $reservation->id, $date_filter, $status_filter); ?>" class="button action-button noshow-button" title="<?php echo esc_attr__('Marquer comme No-Show', 'le-margo'); ?>" onclick="return confirm('<?php echo esc_js(__('Marquer cette réservation comme non-présentée ?', 'le-margo')); ?>');"><span class="dashicons dashicons-marker" style="color:#a00;"></span></a>
                                            <?php endif; ?>
                                            <?php if ($reservation->status !== 'cancelled') : ?>
                                                <a href="<?php echo le_margo_get_action_url('cancel', $reservation->id, $date_filter, $status_filter); ?>" class="button action-button cancel-button" title="<?php echo esc_attr__('Annuler', 'le-margo'); ?>" onclick="return confirm('<?php echo esc_js(__('Êtes-vous sûr de vouloir annuler cette réservation?', 'le-margo')); ?>');"><span class="dashicons dashicons-no"></span></a>
                                            <?php endif; ?>
                                            <a href="<?php echo le_margo_get_action_url('delete', $reservation->id, $date_filter, $status_filter); ?>" class="button action-button delete-button" title="<?php echo esc_attr__('Supprimer', 'le-margo'); ?>" onclick="return confirm('<?php echo esc_js(__('Êtes-vous sûr de vouloir supprimer définitivement cette réservation?', 'le-margo')); ?>');"><span class="dashicons dashicons-trash"></span></a>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
